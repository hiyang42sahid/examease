...
epsilon = 10 #image error sensitivity
 #load tracking tags
tags = [cv2.imread("APP/markers/top_left.png", cv2.IMREAD_GRAYSCALE),
        cv2.imread("APP/markers/top_right.png", cv2.IMREAD_GRAYSCALE),
        cv2.imread("APP/markers/bottom_left.png", cv2.IMREAD_GRAYSCALE),
        cv2.imread("APP/markers/bottom_right.png", cv2.IMREAD_GRAYSCALE)]
...
def ProcessPage(paper):
        ...
        # cv2.imshow("gammam image 1", img1)

        corners = FindCorners(img1) #find the corners of the bubbled area
...
def FindCorners(paper):
    gray_paper = cv2.cvtColor(paper, cv2.COLOR_BGR2GRAY) #convert image of paper to grayscale COLOR_BGR2GRAY 
 
    #scaling factor used later816.0
    ...
    for corner in corners:
        ...
        (corner[0] + int(ratio * 25), corner[1] + int(ratio * 25)), (0, 255, 255), thickness=2, lineType=8, shift=0)
